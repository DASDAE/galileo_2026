...
def _():
    ...
    return dc, get_data_path, get_image_path, get_inventory_path, np, pd
...
@app.cell
def _(enriched, pd):
    # Every downgoing leg in the south drift, split into one patch per hole
    # by expand_by, then each hole's files merged into one patch: time=...
    # merges everything along time (like time=None in notebook 02), with the
    # same forgiving tolerance as above.
    _south_down = (
    ...
    _rows = []
    for _hole_patch in _south_down:
        _strain = _hole_patch.radians_to_strain().convert_units("microstrain")
        # hole_depth is metres down from the collar, so (0, 5) is the top
        # 5 m. The holes differ in depth, so the bottom 5 m is measured from
        # each hole's own deepest channel. select trims the patch, then
        # NumPy's mean averages what is left over time and distance.
        _depth = _hole_patch.get_coord("hole_depth").max()
        _top_mean = _strain.select(hole_depth=(0, 5)).data.mean()
        _bottom_mean = _strain.select(
            hole_depth=(_depth - 5, _depth)
        ).data.mean()
        _rows.append(
            {
                # expand_by stamped the hole's name on the patch attrs.
                "borehole": _hole_patch.attrs.borehole,
                "channels": len(_hole_patch.get_coord("distance")),
                "depth [m]": _depth,
                "top 5 m [µε]": _top_mean,
                "bottom 5 m [µε]": _bottom_mean,
                "top - bottom [µε]": _top_mean - _bottom_mean,
            }
        )
    _table = (
        pd.DataFrame(_rows)
        .sort_values("borehole")
...
def _(mo):
    mo.md(r"""
    Nine holes (S100 to S180) and 167 channels, 17 to 19 per hole. The top and bottom means differ in every hole, although the magnitude and sign vary. Compare the absolute differences rather than ratios: several bottom means are close to zero, so dividing by them would make the contrast look arbitrarily large.

    This is a descriptive comparison of the recorded strain, not evidence by itself that the rock moved more near the top. Coupling changes near the borehole collar, the phase baseline, and interrogator processing could also contribute. Distinguishing those explanations requires a replicated or independently calibrated measurement.
    """)
    return
